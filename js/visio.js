...
(function() {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    // Utiliser la configuration du PDF si disponible, sinon extraire depuis l'URL
    let pdfInfo;
    if (window.pdfConfig) {
      pdfInfo = {
        pdfFileName: window.pdfConfig.fileName,
        title: window.pdfConfig.title
      };
    } else {
      pdfInfo = extractPdfInfoFromUrl();
    }
    ...
    // Mettre à jour la source du PDF dans l'iframe (seulement si pas déjà définie)
    const pdfViewer = document.getElementById('pdf-viewer');

    if (pdfViewer && !pdfViewer.src.includes('.pdf')) {
