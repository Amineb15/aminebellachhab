...
/**
 * Gère le formulaire de contact avec EmailJS
 */
function initContactForm() {
    // Vérifier si EmailJS est chargé
    if (typeof emailjs === 'undefined') {
        console.error('EmailJS n\'est pas chargé. Vérifiez que le script EmailJS est inclus.');
        showNotification('Erreur de configuration : EmailJS non chargé.', 'error');
        return;
    }
    
    // Initialisation d'EmailJS
    try {
        emailjs.init("oj7sMxreOWYNrU0nd");
        console.log('EmailJS initialisé avec succès');
    } catch (error) {
        console.error('Erreur lors de l\'initialisation d\'EmailJS:', error);
        showNotification('Erreur d\'initialisation EmailJS', 'error');
        return;
    }
    ...
    contactForm.addEventListener('submit', async (e) => {
        ...
        try {
            console.log('Tentative d\'envoi avec les paramètres:', {
                serviceId: 'service_m_6emetlc',
                templateId: 'template_ih59kqg',
            ...
            if (error.status === 400) {
                errorMessage = 'Erreur de configuration : Vérifiez vos IDs de service et template EmailJS.';
            } else if (error.status === 401) {
                errorMessage = 'Erreur d\'authentification : Vérifiez votre clé publique EmailJS.';
            } else if (error.status === 402) {
                errorMessage = 'Limite d\'emails atteinte. Veuillez réessayer plus tard.';
            } else if (error.status === 404) {
                errorMessage = 'Service ou template EmailJS non trouvé. Vérifiez vos IDs.';
            } else if (error.text) {
            ...
            showNotification(errorMessage + ' Vous pouvez me contacter directement à aminebellachhab@yahoo.fr', 'error');
        } finally {
            // Réactiver le bouton
            if (submitBtn) {
